...
class Runner(object):
    def __init__(self, config):
        ...
        self.num_agents_level_1 = config['num_agents_level_1']
        self.num_agents_level_2 = config['num_agents_level_2'] * config['num_agents_level_1']
        ...
        # policy network
        self.policy = []
        policy_level_1 = Policy(self.all_args,
                            self.envs.observation_space[0],
                            share_observation_space,
                            self.envs.action_space[0],
                            device = self.device)

        policy_level_2 = Policy(self.all_args,
                            self.envs.observation_space[0],
                            share_observation_space,
                            self.envs.action_space[0],
                            device = self.device)
        
        self.policy.append(policy_level_1)
        self.policy.append(policy_level_2)
        ...
        trainer_level_1 = TrainAlgo(self.all_args, self.policy[0], device = self.device)
        trainer_level_2 = TrainAlgo(self.all_args, self.policy[1], device = self.device)

        self.trainer.append(trainer_level_1)
        self.trainer.append(trainer_level_2)
        
        # buffer
        buffer_level_1 = SharedReplayBuffer(self.all_args,
                                        self.num_agents_level_1,
                                        self.envs.observation_space[0],
                                        share_observation_space,
                                        self.envs.action_space[0])

        buffer_level_2 = SharedReplayBuffer(self.all_args,
                                        self.num_agents_level_2,
                                        self.envs.observation_space[0],
                                        share_observation_space,
                                        self.envs.action_space[0])

        self.buffer.append(buffer_level_1)
        self.buffer.append(buffer_level_2)
